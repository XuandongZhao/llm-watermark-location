...
class WmGenerator():
    ...
    def generate(
        ...
        # set the length of the generated text, controlled dynamically by max_gen_len\max_prompt_size
        total_len = max(self.max_seq_len, max_gen_len + max_prompt_size)

        # create (bsz,total_len) tensor, init: value = pad_id
        tokens = torch.full((bsz, total_len), self.pad_id).to(device).long()
        for k, t in enumerate(prompt_tokens):
            # initialize the token of prompts
            print("len_t: ",len(t)," tokens_len: ",tokens[k].size())
            tokens[k, : len(t)] = torch.tensor(t).long()
            
        #get the mask of tokens
        input_text_mask = tokens != self.pad_id

        # start from the min prompt size, generate text in every place
    ...
    def sample_next(
        ...
        temperature: float = 0.8, # temperature for sampling 较高的值增加随机性，较低的值增加确定性
        top_p: float = 0.95, # top p for sampling 较高top_p使得选概率更大的词
    ) -> torch.LongTensor:
        """ Vanilla sampling with temperature and top p."""
        if temperature > 0:
            probs = torch.softmax(logits / temperature, dim=-1)
            probs_sort, probs_idx = torch.sort(probs, dim=-1, descending=True)
            probs_sum = torch.cumsum(probs_sort, dim=-1) # 累积概率
